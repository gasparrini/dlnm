...
checkcrossbasis <- function(argvar, arglag, addarg) {
  # CHECK LIST FORMAT
  if (!is.list(argvar)) 
    stop("'argvar' must be a list")
  if (!is.list(arglag)) 
    stop("'arglag' must be a list")
  # OLD ARGUMENT type
  ...
  checklag <- function(fun = NULL, df = NULL, knots = NULL, ...) {
    if ((is.null(fun) || fun %in% c("ns", "bs", "strata")) && is.null(knots) && (!is.null(df) && df > 
      1)) 
  ...
  # 'VERY' OLD USAGE
  if (any(c("vartype", "vardf", "vardegree", "varknots", "varbound", "varint", "cen", "cenvalue", "maxlag", 
    "lagtype", "lagdf", "lagdegree", "lagknots", "lagbound", "lagint") %in% addarg)) 
