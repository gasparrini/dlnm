...
getcoef <- function(model, class) {
  # EXTRACT COEF NB: gam, gee AND geeglm HAVE CLASS glm AS WELL
  coef <- if (any(class %in% c("glm", "gam", "coxph"))) 
    coef(model) else if (any(class %in% c("lme", "lmerMod", "glmerMod"))) 
    fixef(model) else tryCatch(coef(model), error = function(w) "error")
