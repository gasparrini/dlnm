...
mkXpred <- function(type, basis, at, predvar, predlag, cen) {
  # CREATE THE MATRIX OF TRANSFORMED CENTRED VARIABLES (DEPENDENT ON TYPE) CREATE VECTORIZED LAGGED VALUES
  varvec <- if (is.matrix(at)) 
    as.numeric(at) else rep(at, length(predlag))
  lagvec <- rep(predlag, each = length(predvar))
  # 
  if (type == "cb") {
    # IF STANDARD CROSS-BASIS, CREATE MARGINAL BASIS AND CALL TENSOR NB: ORDER OF BASIS MATRICES IN TENSOR
    # CHANGED SINCE VERSION 2.2.4 CENTERING APPLIED ONLY MARGINALLY TO VAR DIMENSION
    ...
    ind <- match(c("fun", names(formals(attr(basis, "fun")))), names(attributes(basis)), nomatch = 0)
    ...
    # FINALLY, IF GAM, CALL PredictMat WITH PROPER DATA CENTERING APPLIED TO THE TENSOR PRODUCT (NOT
