...
fci <- function(ci, x, high, low, ci.arg, plot.arg, noeff = NULL) {
  if (ci == "area") {
    ...
    polygon.arg <- modifyList(polygon.arg, list(x = c(x, rev(x)), y = c(high, rev(low))))
    ...
    segments.arg <- modifyList(segments.arg, list(x0 = x - range, y0 = high, x1 = x + range, y1 = high))
    do.call(segments, segments.arg)
    segments.arg <- modifyList(segments.arg, list(x0 = x - range, y0 = low, x1 = x + range, y1 = low))
    do.call(segments, segments.arg)
  } else if (ci == "lines") {
