...
smooth.construct.cb.smooth.spec <- function(object, data, knots) {
  ...
  if (length(object$term) != 2L) 
    stop("'cb' smoother only accepts two terms")
  # CHECK FOR argvar AND arglag OBJECTS IN xt
  argvar <- object$xt$argvar
  arglag <- object$xt$arglag
  # PREVENT USE OF by
  if (object$by != "NA") 
    stop("'by' argument not (yet) applicable with 'cb' smoother")
  # DEFINE ARGUMENTS FOR BUILDING MARGINAL BASES: term, k, fx, bs, m (NO xt NEEDED) TERMS AND DIMENSIONS
  term <- object$term
  dim <- length(term)
  ...
  bs <- object$xt$bs
  if (is.null(bs)) 
    bs <- "ps" else if (!all(bs %in% c("ps", "cr"))) 
  ...
  if (sum(is.na(fx)) || is.null(fx)) 
    fx <- rep(FALSE, 2) else if (length(fx) == 1) 
    fx <- rep(fx, 2)
  # BUILD THE MARGINAL BASES SET EMPTY LISTS TO STORE THE RESULTS
  margin <- dat <- knt <- Xm <- Sm <- list(var = NULL, lag = NULL)
  # SET EMPTY OBJECT TO STORE INFO ON DIMENSIONS, RANK, NULL SPACE NB: SOME LEFT 0 (THEREFORE NOT SET) FOR
  # PARAMETRIC SMOOTHERS
  d <- r <- nr <- rep(0, 2)
  # SET CONSTRAINTS
  mc <- c(TRUE, FALSE)
  C <- matrix(0, 0, 0)
  # MARGINAL BASES IF SMOOTHER NOT DEFINED THROUGH xt LIST, USE THE STANDARD METHOD OTHERWISE, USE onebasis
  ...
  for (i in seq(dim)) {
    ...
    if (is.null(xtarg)) {
      ...
      margin[[i]] <- if (mc[i]) 
        smoothCon(sobj, dat, knt, absorb.cons = TRUE, n = length(dat[[1]]))[[1]] else smooth.construct(sobj, dat, knt)
      Xm[[i]] <- margin[[i]]$X
      if (!fx[i]) 
        Sm[[i]] <- margin[[i]]$S[[1]]
      d[i] <- ncol(margin[[i]]$X)
      ...
      if (i == 2 && (is.null(xtarg$fun) || "intercept" %in% names(formals(xtarg$fun))) && sum(pmatch(names(xtarg), 
  ...
  # BUILD TENSOR AND PENALTY MATRICES TENSOR (USING mgcv FUNCTION) NB: REMOVING NAMES SPEEDS UP THE TENSOR
  ...
  # ADD OTHER INFO AND SET OTHER FEATURES OF TENSOR BUILD THE OBJECT (SOME INFO IN THE ORIGINAL SMOOTH SPEC
  # OBJECT) NB: - np SET TO FALSE (NO REPARAMETERIZATION), SO XP=list() - plot.me DOES NOT ALLOW (YET)
  # PLOTTING USING mgcv FUNCTIONS
  ret <- list(margin = margin, term = term, by = object$by, fx = fx, label = object$label, dim = dim, mp = TRUE, 
    np = FALSE, id = object$id, sp = object$sp, inter = TRUE, mc = mc, plot.me = FALSE, X = X, S = S, 
    C = C, df = ncol(X), null.space.dim = nr, rank = r, XP = list())
