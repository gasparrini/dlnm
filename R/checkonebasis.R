...
checkonebasis <- function(fun, args, cen) {
  ...
  if (!is.character(fun)) 
    stop("'fun' must be a string referring to a function")
  # FS: looking up the function object referenced by the fun-string seems to 
  # break in R CMD check for the examples with user-defined functions, had to
  # set them to \dontrun, seemed easier than diving into  these nasty scoping 
  # issues 
  if (all(names(formals(fun)) != "x")) 
    stop("'fun' must contain argument 'x'")
  # CHECK CENTERING, MOVED TO PREDICTION NOW, AND REMOVE FROM ARGUMENTS ALSO, SET TO NULL FOR
