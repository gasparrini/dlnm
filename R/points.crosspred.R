...
#' @export
points.crosspred <- function(x, ptype, var = NULL, lag = NULL, ci = "n", ci.arg, ci.level = x$ci.level, cumul = FALSE, 
  exp = NULL, ...) {
  # 
  if (class(x) != "crosspred") 
    stop("'x' must be of class 'crosspred'")
  ci <- match.arg(ci, c("area", "bars", "lines", "n"))
  # SETING DEFAULT FOR ptype: OVERALL FOR NO LAG, SLICES FOR VAR/LAG, OTHERWISE 3D
  if (missing(ptype)) {
  ...
  # SET THE Z LEVEL EQUAL TO THE STORED IN OBJECT IF NOT PROVIDED
  ...
  if (ptype == "slices") {
    ...
    if (!is.null(lag)) {
      ...
      fci(ci = ci, x = x$predvar, high = x$mathigh[, xlag], low = x$matlow[, xlag], ci.arg, plot.arg)
      plot.arg <- modifyList(plot.arg, c(list(x = x$predvar, y = x$matfit[, xlag])))
    ...
    if (!is.null(var)) {
      ...
      plot.arg <- modifyList(plot.arg, list(...))
      # PLOT CONFIDENCE INTERVALS (IF ANY)
      fci(ci = ci, x = seqlag(x$lag, x$bylag), high = x$mathigh[xvar, ], low = x$matlow[xvar, ], ci.arg, 
        plot.arg)
      plot.arg <- modifyList(plot.arg, c(list(x = seqlag(x$lag, x$bylag), y = x$matfit[xvar, ])))
      do.call("points", plot.arg)
    }
  }
  # OVERALL
  if (ptype == "overall") {
    ...
    fci(ci = ci, x = x$predvar, high = x$allhigh, low = x$alllow, ci.arg, plot.arg)
