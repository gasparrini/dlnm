...
summary.crossreduce <- function(object, ...) {
  # 
  cat("REDUCED FIT\n")
  cat("type:", object$type, "\n")
  cat("dimension:", ifelse(object$type != "var", "predictor", "lag"), "\n")
  if (object$type != "overall") 
    cat("value:", ifelse(object$type == "lag", paste("lag", object$value), object$value), "\n")
  ...
  if (object$type != "var") 
    cat("range:", min(object$predvar), "to", max(object$predvar), "\n")
  if (object$type == "var") 
    cat("lag period:", object$lag, "\n")
  if (object$type != "var") 
    cat("values:", length(object$predvar), "\n")
  if (object$type == "var") 
    cat("by:", length(object$bylag), "\n")
