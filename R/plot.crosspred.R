...
#' for(i in 1:3) {
#'   lines(pred3.temp, 'slices', var=c(0,27,33)[i], col=i+1, lwd=1.5)
#' }  
...
#' 
plot.crosspred <- function(x, ptype, var = NULL, lag = NULL, ci = "area", ci.arg, ci.level = x$ci.level, 
  cumul = FALSE, exp = NULL, ...) {
  # 
  if (class(x) != "crosspred") 
  ...
  # COMPUTE OUTCOMES CUMULATIVE IF CUMUL==T
  if (cumul == TRUE) {
    x$matfit <- x$cumfit
    x$matse <- x$cumse
  }
  ...
  if (ptype == "slices") {
    ...
    if (length(lag) + length(var) > 1) {
      layout(matrix(1:(length(var) + length(lag)), ncol = sum(!is.null(var), !is.null(lag))))
      grey <- grey(0.8)
    ...
    if (!is.null(lag)) {
      ...
      for (i in seq(along = lag)) {
        ...
        plot.arg <- list(type = "l", xlab = "Var", ylab = "Outcome", ylim = c(min(x$matlow[, xlag]), 
          max(x$mathigh[, xlag])), bty = "l")
        if (length(lag) + length(var) > 1) 
          plot.arg$cex.axis <- 0.7
        plot.arg <- modifyList(plot.arg, list(...))
        # SET CONFIDENCE INTERVALS
        ci.list <- list(panel.first = call("fci", ci = ci, x = x$predvar, high = x$mathigh[, xlag[i]], 
          low = x$matlow[, xlag[i]], ci.arg, plot.arg, noeff = noeff))
        plot.arg <- modifyList(plot.arg, c(ci.list, list(x = x$predvar, y = x$matfit[, xlag[i]])))
        if (length(lag) + length(var) > 1) {
          plot.arg$main <- ""
          plot.arg$xlab <- "Var"
        }
    ...
    if (!is.null(var)) {
      ...
      for (i in seq(along = var)) {
        # SET DEFAULT VALUES IF NOT INCLUDED BY THE USER
        plot.arg <- list(type = "l", xlab = "Lag", ylab = "Outcome", ylim = c(min(x$matlow[xvar, 
          ]), max(x$mathigh[xvar, ])), bty = "l")
        if (length(lag) + length(var) > 1) 
          plot.arg$cex.axis <- 0.7
        plot.arg <- modifyList(plot.arg, list(...))
        # SET CONFIDENCE INTERVALS
        ci.list <- list(panel.first = call("fci", ci = ci, x = seqlag(x$lag, x$bylag), high = x$mathigh[xvar[i], 
          ], low = x$matlow[xvar[i], ], ci.arg, plot.arg, noeff = noeff))
        plot.arg <- modifyList(plot.arg, c(ci.list, list(x = seqlag(x$lag, x$bylag), y = x$matfit[xvar[i], 
          ])))
        if (length(lag) + length(var) > 1) {
          plot.arg$main <- ""
          plot.arg$xlab <- "Lag"
  ...
  if (ptype == "overall") {
    # SET DEFAULT VALUES IF NOT INCLUDED BY THE USER
    plot.arg <- list(type = "l", ylim = c(min(x$alllow), max(x$allhigh)), xlab = "Var", ylab = "Outcome", 
      bty = "l")
    plot.arg <- modifyList(plot.arg, list(...))
    # SET CONFIDENCE INTERVALS
    ci.list <- list(panel.first = call("fci", ci = ci, x = x$predvar, high = x$allhigh, low = x$alllow, 
      ci.arg, plot.arg, noeff = noeff))
    plot.arg <- modifyList(plot.arg, c(ci.list, list(x = x$predvar, y = x$allfit)))
    # PLOT
    do.call("plot", plot.arg)
  }
  # CONTOURPLOT
  if (ptype == "contour") {
    ...
    filled.contour(x = x$predvar, y = seqlag(x$lag, x$bylag), z = x$matfit, col = col, levels = levels, 
      ...)
  }
  # 3-D
  if (ptype == "3d") {
    ...
    # SET DEFAULT VALUES IF NOT INCLUDED BY THE USER
    plot.arg <- list(ticktype = "detailed", theta = 210, phi = 30, xlab = "Var", ylab = "Lag", zlab = "Outcome", 
      col = "lightskyblue", zlim = c(min(x$matfit), max(x$matfit)), ltheta = 290, shade = 0.75, r = sqrt(3), 
      d = 5)
    plot.arg <- modifyList(plot.arg, list(...))
    plot.arg <- modifyList(plot.arg, list(x = x$predvar, y = seqlag(x$lag, x$bylag), z = x$matfit))
    # PLOT
